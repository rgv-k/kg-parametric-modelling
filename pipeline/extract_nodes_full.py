...
def main():
    ...
    # 2. Check for Partial Progress (The Resume Logic)
    if os.path.exists(PARTIAL_FILE):
        ...
        try:
            ...
            # Determine where we left off based on how many nodes we have
            # Note: This is an approximation. Since 1 term can produce multiple nodes (polysemy),
            # we need to be careful. A safer way is to check the 'term' of the last node
            # and find its index in the source glossary.
            
            if all_nodes:
                last_term = all_nodes[-1]['term']
                # Find the index of this term in the original glossary
                for i, item in enumerate(glossary_data):
                    if item['term'] == last_term:
                        # We start from the NEXT item to be safe
                        start_index = i + 1
                        break
    ...
    for i in tqdm(range(start_index, total_terms, BATCH_SIZE), desc="Extracting Nodes"):
        ...
        # Save to PARTIAL file every batch now, to be safer
